class SRP_ActionCookDrugs extends ActionSRPVariantIdOption
{	
  void SRP_ActionCookDrugs()
	{
  ...
  override void OnActionInfoUpdate( PlayerBase player, ActionTarget target, ItemBase item )
	{
    SRP_DrugWorkbench craftingWorkbench;    
  ...
  override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
    ...
    if (Class.CastTo(craftingWorkbench, action_data.m_Target.GetObject()))
    {
      ...
      if (newItem.GetItemClassName().Contains("Tainted"))
      {
        float chanceToExplode = Math.RandomFloat01();
        // 15% chance to explode
        if (chanceToExplode > 0.85)
        {
          craftingWorkbench.AddHealth(-10);
          craftingWorkbench.GetInventory().CreateInInventory("SRP_DrugExplosion");
        }
      }
    }		
	}
