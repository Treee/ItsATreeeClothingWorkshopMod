class SRP_AlcoholismNotfr: NotifierBase
{
  // decrease
  ...
  // increase
	...
	void SRP_AlcoholismNotfr(NotifiersManager manager)
	{
	}

	override int GetNotifierType()
	{
		return SRP_eNotifiers.SRP_NTF_ALCOHOLISM;
	}
	...
	override void DisplayTendency(float delta)
	{
		...
		EStatLevels alcoholism_level = m_Player.GetStatLevelAlcoholism();
		DisplayElementTendency dis_elm = DisplayElementTendency.Cast(GetVirtualHud().GetElement(SRP_eDisplayElements.SRP_DELM_TDCY_ALCOHOLISM));
		
		if( dis_elm )
		{
			dis_elm.SetTendency(tendency);
			dis_elm.SetSeriousnessLevel(alcoholism_level);
	...
	{
		return m_Player.GetStatAlcoholism().Get();
	}
};