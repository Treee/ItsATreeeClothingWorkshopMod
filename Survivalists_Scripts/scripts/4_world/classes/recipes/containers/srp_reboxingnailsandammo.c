...
class Craft_SRP_SmallProtectorCase_Armband extends RecipeBase 
{
	override void Init()
	{
		m_Name = "Make Armband/Flag Box";	// action name in game
		m_IsInstaRecipe = false;	// should this recipe be performed instantly without animation
		m_AnimationLength = 1;		// animation length in relative time units
		m_Specialty = 0;			// softskills modifier. value > 0 for roughness, value < 0 for precision
		...
		//ingredient 2	
		InsertIngredient(1,"Armband_ColorBase"); //  secondary ingredient
    ...
    //result1
		AddResult("SRP_SmallProtectorCase_FlagArmbandBox");//add results here
  ...
  override bool CanDo(ItemBase ingredients[], PlayerBase player)
	{
    SRP_SmallProtectorCase_FlagArmbandBox protectorCase = SRP_SmallProtectorCase_FlagArmbandBox.Cast(ingredients[0]);
    if (protectorCase)
    {
      return false;
    }
