	...
	override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	{
        if (ingredients[0].IsCraftingKit())
        {
            ...
            if (!Class.CastTo(battery, ingredients[0].GetItemOnSlot("TruckBattery")))
                return false;
            if (battery.GetCompEM().GetEnergy() < GetRequiredBatteryQuantity())
                return false;
            ...
            ItemBase copperWire;
            if (!Class.CastTo(copperWire, ingredients[0].GetItemOnSlot("SRP_CopperWire1")))
                return false;
            if (copperWire.GetQuantity() < GetRequiredCopperQuantity())
                return false;

            ItemBase wire1;
            if (!Class.CastTo(wire1, ingredients[0].GetItemOnSlot("SRP_StackedElectricalWire1")))
                return false;
            ...
            ItemBase wire2;
            if (!Class.CastTo(wire2, ingredients[0].GetItemOnSlot("SRP_StackedElectricalWire2")))
                return false;
