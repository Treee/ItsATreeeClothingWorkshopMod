class Craft_SRP_HandPlane_Crude extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Craft Crude Hand Plane";
		...
		//result1
		AddResult("SRP_HandPlane_Crude");//add results here
	...
	{
		Debug.Log("Craft_SRP_HandPlane_Crude: Recipe Do method called","recipes");
	}
};

class Craft_SRP_HandPlane_Metal extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Craft Metal Hand Plane";
		m_IsInstaRecipe = false;//should this recipe be performed instantly without animation
		m_AnimationLength = 2;//animation length in relative time units
		m_Specialty = 0.02;// value > 0 for roughness, value < 0 for precision		
		...
		//result1
		AddResult("SRP_HandPlane_Metal");//add results here
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		Debug.Log("Craft_SRP_HandPlane_Metal: Recipe Do method called","recipes");
	}
};
