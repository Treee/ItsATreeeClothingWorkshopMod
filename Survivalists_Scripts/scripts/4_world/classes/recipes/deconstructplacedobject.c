...
{	
    ...
    TurnItemIntoItemLambda_KitDeployment_DamageKit lambda = new TurnItemIntoItemLambda_KitDeployment_DamageKit(ingredients[0], newItemName, player, player.GetPosition());
...
{	
  ...
  override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
		...
		if (ingredients[0].CanBeDeconstructed() && ingredients[0].IsEmpty())
    {
      ...
      {
        if (tool.HasBatteryAttached() && tool.HasRequiredEnergy(250))
        {
          return true;
        }
      }
...
class Deconstruct_PlacedObjectKit_AdvancedCrude extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Dismantle to Kit - Crude (Low Power)";    
		m_IsInstaRecipe = false;//should this recipe be performed instantly without animation
		m_AnimationLength = 2;//animation length in relative time units
		m_Specialty = 0.02;// value > 0 for roughness, value < 0 for precision
  ...
  override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
		...
		if (ingredients[0].CanBeDeconstructed() && ingredients[0].IsEmpty())
    {
      ...
      if (Class.CastTo(tool, ingredients[1]))
      {
        ...
        else if (!tool.HasBatteryAttached())
        {
          return true;
        }
      }
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		Debug.Log("Deconstruct_PlacedObjectKit_AdvancedCrude: Recipe Do method called. Type: " +ingredients[0].GetType(),"recipes");
   
    TurnItemIntoItemLambda_KitDeployment_DamageKit lambda = new TurnItemIntoItemLambda_KitDeployment_DamageKit(ingredients[0], ingredients[0].GetKitName(), player, player.GetPosition());
    lambda.SetTransferParams(false, false);
    MiscGameplayFunctions.TurnItemIntoItemEx(player, lambda);
