...
class Craft_SRP_CrushStoneChunk_JewlingTool extends RecipeBase
{	
	override void Init()
	{
		...
		//ingredient 2
		InsertIngredient(1,"LeatherSewingKit_JewelCuttingTools");//you can insert multiple ingredients this way
    ...
    if (chance >= 0.80)
    {
      string randomGemstone = string.Format("SRP_Mining_Crystal_%1_%2", GetCrystalTier().GetRandomElement(), GetCrystalTypes().GetRandomElement());
      ItemBase gemstone = ItemBase.Cast(GetGame().CreateObjectEx(randomGemstone, player.GetPosition(), false));      
      gemstone.SetQuantity(Math.RandomIntInclusive(1,2));
      results.Insert(gemstone);
    }
		Debug.Log("Craft_SRP_CrushStoneChunk_JewlingTool: Recipe Do method called","recipes");
	}
...
class Craft_SRP_RefineUnCutGem_JewlCuttingTool extends RecipeBase
{	
	override void Init()
	{
    ...
    else
    {
      m_ResultSetHealth[0] = -1;
		...
		Debug.Log("Craft_SRP_RefineUnCutGem_JewlCuttingTool: Recipe Do method called","recipes");
	}
};
...
class Craft_SRP_MetalBucket extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Make Bucket";
		...
		m_MinQuantityIngredient[0] = 12;//-1 = disable check
		m_MaxQuantityIngredient[0] = -1;//-1 = disable check
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"SRP_Mining_StoneChunk_Clay");//you can insert multiple ingredients this way
		...
		//ingredient 2
		InsertIngredient(1,"SRP_Mining_StoneChunk_Clay");//you can insert multiple ingredients this way
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		Debug.Log("Craft_SRP_MetalBucket: Recipe Do method called","recipes");
	}
};
