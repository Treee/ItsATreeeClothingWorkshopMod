...
class SRP_AdvancedWorkbench extends SRP_DeployableContainer_Base
{
  override void SetActions()
	{
		super.SetActions();
		AddAction(ActionInteractWithAmmoCraftingWorkbench);
	}
  ...
  string HasCorrectAmmoComponents()
  {
    ItemBase casings = GetItemOnSlot("SRP_AmmoCasings");
    if (casings == NULL)
    {
      return "";
    }
    ItemBase gunpowder = GetItemOnSlot("SRP_AmmoGunpowder");
    if (gunpowder == NULL)
    {
      return "";
    }
    ItemBase bullets = GetItemOnSlot("SRP_AmmoBullets");
    if (bullets == NULL)
    {
      return "";
    }
    // quantity check
    if (casings.GetQuantity() < 10 || gunpowder.GetQuantity() < 10 || bullets.GetQuantity() < 10)
    {
      return "";
    }
    if (casings.GetType().Contains("Blackout") || bullets.GetType().Contains("Blackout"))
    {
      return "";
    }
    TStringArray parts = new TStringArray;
    casings.GetType().Split( "_", parts );
    if (parts.Count() != 3)
    {
      return "";
    }

    return BuildAmmoNameFromParts(parts[2]);
  }

  void RemoveAmmoCraftingComponents()
  {
    ItemBase casings = GetItemOnSlot("SRP_AmmoCasings");
    if (casings == NULL)
    {
    ...
    casings.AddQuantity(-10);
    gunpowder.AddQuantity(-10);
    bullets.AddQuantity(-10);
  }

  string BuildAmmoNameFromParts(string part)
  {
    if (part == "556")
    {
    ...
    // Print("parts: " + part);
    return string.Format("Ammo_%1",part);
  }
