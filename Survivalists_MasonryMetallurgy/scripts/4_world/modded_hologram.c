modded class Hologram
{
  override string ProjectionBasedOnParent()
    ...
    if (itemInHands.IsInherited(SRP_MasonryMetallurgy_KitBase))
    {
      // Print("Inherits from my kit");
      SRP_MasonryMetallurgy_KitBase srpKit = SRP_MasonryMetallurgy_KitBase.Cast(itemInHands);
    ...
    // If we are placing our kits
		if ( entity_for_placing.IsInherited( SRP_MasonryMetallurgy_KitBase ))
		{
  ...
  override void EvaluateCollision(ItemBase action_item = null)
    ...
    EntityAI itemInHands = m_Player.GetItemInHands();
    if (itemInHands && itemInHands.IsInherited(SRP_MasonryMetallurgy_KitBase)) // if the item in our hands is our kit
    {
      // ignore collision
