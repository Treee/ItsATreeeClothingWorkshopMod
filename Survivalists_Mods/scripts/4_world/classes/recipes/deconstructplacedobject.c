...
class Deconstruct_IntermediaryWoodenFurnitureKit extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Dismantle";
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		Debug.Log("Deconstruct_IntermediaryWoodenFurnitureKit: Recipe Do method called. Type: " +ingredients[0].GetType(),"recipes");
    // SRP_KitBase.Cast( GetGame().CreateObjectEx(ingredients[0].GetType()+"_Kit", player.GetPosition(), false) );
	}
};
...
class Deconstruct_PlacedLetters extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Dismantle";
    ...
    InsertIngredient(1,"Pliers");
		...
		//result1
    AddResult("SRP_Letter_");//add results here
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		Debug.Log("Deconstruct_PlacedLetters: Recipe Do method called. Type: " +ingredients[0].GetType(),"recipes");
    // SRP_KitBase.Cast( GetGame().CreateObjectEx(ingredients[0].GetType()+"_Kit", player.GetPosition(), false) );
	}
};