modded class Hologram
{
  override string ProjectionBasedOnParent()
    ...
    if (itemInHands.IsInherited(SRP_RadioElectronics_KitBase))
    {
      // Print("Inherits from my kit");
      SRP_RadioElectronics_KitBase srpKit = SRP_RadioElectronics_KitBase.Cast(itemInHands);
  ...
  override void EvaluateCollision(ItemBase action_item = null)
    ...
    EntityAI itemInHands = m_Player.GetHumanInventory().GetEntityInHands();
    if (itemInHands && itemInHands.IsInherited(SRP_RadioElectronics_KitBase)) // if the item in our hands is our kit
    {
      // ignore collision
