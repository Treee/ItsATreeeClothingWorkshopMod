class Craft_SRP_UnSealedLetter extends RecipeBase
{
	override void Init()
	{
		m_Name = "Make Envelope";
		...
		//result1
		AddResult("SRP_UnSealedLetter");//add results here
  ...
  override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	...
	{
		Debug.Log("Craft_SRP_UnSealedLetter: Recipe Do method called","recipes");
	}
};

class Craft_SRP_OpenSealedLetter extends RecipeBase
{
	override void Init()
	{
		m_Name = "Open Sealed Letter";
		m_IsInstaRecipe = false;//should this recipe be performed instantly without animation
		m_AnimationLength = 1;//animation length in relative time units
		m_Specialty = 0.02;// value > 0 for roughness, value < 0 for precision
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"SRP_SealedLetter_ColorBase");//you can insert multiple ingredients this way

		m_IngredientAddHealth[0] = -100;// 0 = do nothing
		m_IngredientSetHealth[0] = -1; // -1 = do nothing
		m_IngredientAddQuantity[0] = 0;// 0 = do nothing
		...
		//result1
		// AddResult("SRP_SealedLetter_");//add results here
  ...
  override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		Debug.Log("Craft_SRP_OpenSealedLetter: Recipe Do method called","recipes");
	}
};