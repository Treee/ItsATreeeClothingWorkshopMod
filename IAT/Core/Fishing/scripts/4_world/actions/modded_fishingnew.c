  ...
  override void CreateActionComponent()
    ...
    // if server side
      ...
      // if the hook is not empty
        ...
        // damage the hook
				m_ActionDataFishing.m_Bait.AddHealth(-m_ActionDataFishing.FISHING_DAMAGE);
        ItemBase lure = ItemBase.Cast(m_ActionDataFishing.m_MainItem.FindAttachmentBySlotName("IAT_FishingLure"));
        if (lure)
        {
      ...
      // if we roll 20 or higher
			...
			{
        bool hasLure = (ItemBase.Cast(m_ActionDataFishing.m_MainItem.FindAttachmentBySlotName("IAT_FishingLure")) != NULL);
